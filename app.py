...
def process_single_image(img, image_name):
    """Process a single image with all 6 enhancement methods using Tesseract"""
    ...
    try:
        for idx, method in enumerate(methods):
            ...
            # Extract text using Tesseract with different PSM modes and configurations
            try:
                # Try multiple Tesseract configurations for better results
                configs = [
                    '--psm 6 -c tessedit_char_whitelist=0123456789.mMcCeEtTrRsS ',  # Single block text with numbers and units
                    '--psm 7 -c tessedit_char_whitelist=0123456789.mMcCeEtTrRsS ',  # Single text line
                    '--psm 8 -c tessedit_char_whitelist=0123456789.mMcCeEtTrRsS ',  # Single word
                    '--psm 13 -c tessedit_char_whitelist=0123456789.mMcCeEtTrRsS ', # Raw line
                ]
                ...
                # If no specific config worked, try default
...
def main():
    ...
    <div class="main-header">
        ...
        <div style="background: rgba(255,255,255,0.2); padding: 10px; border-radius: 10px; margin: 10px 0;">
            🚀 Powered by Tesseract OCR - Advanced Dual Image Analysis
        </div>
    ...
    with st.sidebar:
        ...
        - ⚡ **Multiple OCR Modes**: Different PSM modes for better accuracy
        """)
        ...
        - Processing may take 1-2 minutes per image
    ...
    if uploaded_file1 is not None and uploaded_file2 is not None:
        if st.button("🔍 Analyze Both Images", key="analyze_btn"):
            try:
                ...
                st.info("⏳ This may take 1-3 minutes for all 6 methods per image. Please be patient...")
